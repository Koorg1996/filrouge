...
kmeans = KMeans(n_clusters=4).fit(tableau_movies)
...
tableau_users = df_score.pivot(index = 'userId', columns = 'Kmeans_movies_cluster').reset_index()
tableau_users = tableau_users.replace(np.nan, 0)
#tableau_users

df_kmeans_users = tableau_users['score']
df_kmeans_users
...
kmeans = KMeans(n_clusters=6).fit(df_kmeans_users)
centroids = kmeans.cluster_centers_
